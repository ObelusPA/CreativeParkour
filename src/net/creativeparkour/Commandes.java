			...
			try {
				if (sender instanceof Player)
				{
					...
					if (args.length > 0 || cmd.getName().equalsIgnoreCase("cpd"))
					{
						...
						{
							if (Config.isBanned(p) && !args[0].equalsIgnoreCase("pardon"))
							{
										...
										if (args.length > 1)
										{
											...
											else
											{
												if (Config.isBanned(uuidP1))
													p.sendMessage(Config.prefix() + ChatColor.RED + Langues.getMessage("commands.ban error"));
												else
												{
													...
													p.sendMessage(Config.prefix() + ChatColor.GREEN + Langues.getMessage("commands.ban banned").replace("%player", NameManager.getNomAvecUUID(uuidP1)).replace("%uuid", uuidP1.toString()));
												}
											}
										}
										else
								...
								else if (args[0].equalsIgnoreCase("pardon") || args[0].equalsIgnoreCase(Langues.getMessage("commands.pardon")))
								{
									...
									if (p.hasPermission("creativeparkour.manage"))
									{
										if (args.length > 1)
										{
											...
											{
												if (!Config.isBanned(uuidP1))
													p.sendMessage(Config.prefix() + ChatColor.RED + Langues.getMessage("commands.pardon error"));
												else
												{
													Config.getConfJoueur(uuidP1.toString()).set("banned", false);
													Config.saveConfJoueur(uuidP1.toString());
													p.sendMessage(Config.prefix() + ChatColor.GREEN + Langues.getMessage("commands.pardon pardoned").replace("%player", NameManager.getNomAvecUUID(uuidP1)).replace("%uuid", uuidP1.toString()));
												}
											}
											else
												p.sendMessage(Config.prefix() + ChatColor.RED + Langues.getMessage("player offline"));
